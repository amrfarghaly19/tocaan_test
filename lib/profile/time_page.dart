...
class _SchedulePageState extends State<SchedulePage> {
  ...
  Future<void> submitSchedule() async {
    final url = Uri.parse('https://api.example.com/schedule');
    ...
    final payload = {
      ...
      "fitness_level": "beginner", // Replace with your logic
      "goal": "weight_loss",      // Replace with your logic
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('An error occurred while updating the schedule.')),
      );
    }
  }
...
class _SchedulePageState extends State<SchedulePage> {
  ...
  Future<void> submitSchedule() async {
    final url = Uri.parse('${Config.baseURL}/settings/time');
    ...
    final payload = {
      ...
      "work_start_time": startWorkTime != null ? _formatTimeOfDay(startWorkTime!) : "00:00:00",
      "work_end_time": endWorkTime != null ? _formatTimeOfDay(endWorkTime!) : "00:00:00",
    /*  "fitness_level": "${fitnessLevels[selectedFitnessLevelIndex]["name"]}", // Replace with actual value
      "goal": "${goals.firstWhere((g) => g["key"] == selectedGoalKey)["name"]}",      // Replace with actual value
      "exercise_days": exerciseDays,*/
    };
                ...
                Theme(
                  data: Theme.of(context).copyWith(
                    sliderTheme: SliderTheme.of(context).copyWith(
                      activeTrackColor: Colors.red, // Active track color
                      // inactiveTrackColor: Colors.grey.shade300, // Inactive track color
                      thumbColor: Colors.red, // Thumb color
                      overlayColor: Colors.red.withOpacity(0.2), // Overlay color
                      valueIndicatorTextStyle: TextStyle( // Label text style
                        color: Colors.white, // Set label text color
                        fontWeight: FontWeight.bold,
                      ),
                      valueIndicatorColor: Colors.red, // Background color for the label
                    ),
                  ),
                  child: Slider(
                    activeColor: Colors.red, // Color for the active portion of the slider
                    // inactiveColor: Colors.grey.shade300, // Color for the inactive portion of the slider
           ...
           /*     Text(
                ...
                ElevatedButton(
                  ...
                  child: Text(
                    ...
                    'Selected Goal: ${goals.firstWhere((g) => g["key"] == selectedGoalKey)["name"]}':
                    'Select Goal',
                ...
                if (selectedFitnessLevelKey != null || selectedGoalKey != null)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      if (selectedFitnessLevelKey != null)
                        Text(
                          'Selected Fitness Level Key: $selectedFitnessLevelKey',
                          style: TextStyle(color: Colors.white),
                        ),
                      if (selectedGoalKey != null)
                        Text(
                          'Selected Fitness Goal Key: $selectedGoalKey',
                          style: TextStyle(color: Colors.white),
                        ),
                    ],
                  ),*/
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          border: Border.all(
                            color: selectedGoalKey == goal["key"] ? Colors.red : Colors.grey,
                            width: 2,
                          ),
                          borderRadius: BorderRadius.circular(8),
                        ),
